...
type Action = { type: 'reset' } | { type: 'error'; error: string } | { type: 'user'; message: Message; online: boolean } | { type: 'event'; event: ServerEvent };
export function sessionReducer(state: State, action: Action): State {
  ...
  if (action.type === 'user') {
    ...
    return { ...state, messages: [...state.messages, action.message], busy: action.online, error: '' };
  }
  ...
  if (event.type === 'assistant.text' && typeof event.payload.text === 'string') {
    const message: Message = { id: `assistant-${event.turn_id ?? event.event_id}`, role: 'assistant', text: event.payload.text };
  ...
  if (event.type === 'error') { next.busy = false; next.error = 'Сервер не смог обработать реплику. Попробуйте позже.'; }
  return next;
}
