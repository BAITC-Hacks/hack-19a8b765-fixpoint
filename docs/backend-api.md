...
Снимок: `GET /api/sessions/{id}` для активных и сохранённых разговоров. Содержит
`schema_version`, даты UTC, `status` (active/closed/interrupted), `close_reason`,
`state`, `mock_backend`, `action_audit`, `request_ids`, все `turns` и `traces`.
В `turns` сохраняются ответ сервера, входные метаданные, время начала/завершения,
текст ответа и события без base64-аудио. `in_progress` фиксирует принятый запрос,
который мог не завершиться при аварии. Архив не восстанавливает исполнение операции.

- `GET /api/sessions?limit=50&offset=0` → `{sessions:[...], total}`; limit 1–100,
  сортировка по последнему обновлению. Метаданные включают `turn_count`, `preview`,
  `pending_request`, даты и статус.
- `GET /api/sessions/{id}/trace` → все трассировки разговора, без ограничения 20.
- `GET /api/sessions/{id}/export` → полный JSON с Content-Disposition attachment.
- `POST /api/sessions/{id}/close` → завершить живую сессию, сохранить архив.
- `DELETE /api/sessions/{id}` → совместимый алиас завершения, **не удаляет архив**.

Хранилище: `runtime/sessions/<UUID>.json`, атомарная замена файла; настройка
`SESSION_ARCHIVE_DIR`. После часа простоя живая сессия закрывается с причиной
idle_timeout. Перезапуск оставляет архивы, открытые разговоры становятся interrupted.
Новые ходы в завершённый/прерванный разговор отклоняются с 409. Активные моки нового
разговора начинаются с исходных данных. Это локальное демо без системы аккаунтов,
поддерживается один процесс сервера. Архив не содержит настроек провайдеров/ключей
или аудиобайлов. При ошибке сохранения до обработки действия блокируются; при ошибке
после выполнения результат остаётся в памяти и клиент получает явную ошибку сохранения.
