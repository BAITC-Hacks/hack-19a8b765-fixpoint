...
if not health['llm_ready']:
    st.warning('Демонстрационный режим: ключ LLM-провайдера не подключён. Маршрутизация LLM и распознавание речи недоступны. Ручной выбор ниже проверяет исполнителя.')
else:
